...
const { ETIQUETA_ROL, dibujarImagenSiExiste } = require('./helpers');

// datos = {
//   institucion, persona: { nombreCompleto, tipoDocumento, documento, rol, foto, grado, grupoNombre, cargo },
//   anio
// }
...
const generarCarnetPDF = (res, datos) => {
  const { institucion, persona, anio } = datos;
  ...
  const color = colorPorRol[persona.rol] || '#1d4ed8';
  ...
  doc.font('Helvetica-Bold').fontSize(7.5).fillColor(color)
    .text(ETIQUETA_ROL[persona.rol] || persona.rol, 82, 62, { width: 155 });

  doc.font('Helvetica').fontSize(7.5).fillColor('#000000');
  doc.text(`${persona.tipoDocumento || ''} ${persona.documento || '-'}`, 82, 75, { width: 155 });

  if (persona.rol === 'estudiante') {
    doc.text(`Grado ${persona.grado ?? '-'}  Grupo ${persona.grupoNombre || '-'}`, 82, 88, { width: 155 });
  } else if (persona.cargo) {
    doc.text(persona.cargo, 82, 88, { width: 155 });
  }
